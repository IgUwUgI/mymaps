export const planeIcon = L.icon({
    iconUrl: "../assets/plane.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon = L.icon({
    iconUrl: "../assets/ville.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon0 = L.icon({
    iconUrl: "../assets/ville0.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon1 = L.icon({
    iconUrl: "../assets/ville1.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon2 = L.icon({
    iconUrl: "../assets/ville2.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon3 = L.icon({
    iconUrl: "../assets/ville3.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const cityIcon4 = L.icon({
    iconUrl: "../assets/ville4.png",

    iconSize: [38, 38],
    iconAnchor: [19, 19],
    popupAnchor: [-3, -76]
})

export const carMarker = L.icon({
    iconUrl: "../assets/carMarker.png",
    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})

export const carMarkerBig = L.icon({
    iconUrl: "../assets/carMarker.png",
    iconSize: [45, 60],
    iconAnchor: [23, 60],
    popupAnchor: [-3, -76]
})

export const busMarker = L.icon({
    iconUrl: "../assets/busMarker.png",

    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})

export const busMarkerBig = L.icon({
    iconUrl: "../assets/busMarker.png",

    iconSize: [45, 60],
    iconAnchor: [23, 60],
    popupAnchor: [-3, -76]
})

export const boatMarker = L.icon({
    iconUrl: "../assets/boatMarker.png",

    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})

export const boatMarkerBig = L.icon({
    iconUrl: "../assets/boatMarker.png",

    iconSize: [45, 60],
    iconAnchor: [23, 60],
    popupAnchor: [-3, -76]
})

export const hikeMarker = L.icon({
    iconUrl: "../assets/hikeMarker.png",
    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})

export const hikeMarkerBig = L.icon({
    iconUrl: "../assets/hikeMarker.png",
    iconSize: [45, 60],
    iconAnchor: [23, 60],
    popupAnchor: [-3, -76]
})

export const trainMarker = L.icon({
    iconUrl: "../assets/trainMarker.png",

    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})


export const bikeMarker = L.icon({
    iconUrl: "../assets/bikeMarker.png",

    iconSize: [30, 40],
    iconAnchor: [15, 40],
    popupAnchor: [-3, -76]
})

export const bikeMarkerBig = L.icon({
    iconUrl: "../assets/bikeMarker.png",
    iconSize: [45, 60],
    iconAnchor: [23, 60],
    popupAnchor: [-3, -76]
})